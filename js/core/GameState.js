    ...
    emitSpecificEvents(path, newValue, oldValue) {
        ...
        if (path === 'hero.overheat') {
            ...
            if (newValue >= this.state.hero.maxOverheat) {
                eventBus.emit(GameEvents.OVERHEAT_THRESHOLD, { type: 'meltdown' });
            }
        }
        ...
        if (path === 'credits') {
            if (newValue > oldValue) {
                eventBus.emit(GameEvents.CREDITS_GAINED, { amount: newValue - oldValue });
            } else {
                eventBus.emit(GameEvents.CREDITS_SPENT, { amount: oldValue - newValue });
            }
        }
