 ...
 * @version 0.3.0 - Added Lyria fallback data, varra/bookend event paths
 */
...
class DataLoader {
    constructor() {
        ...
        this.fallbackData = {
            ...
            cards: [
                { id: 'strike', name: 'Strike', type: 'attack', cost: 1, rarity: 'starter', description: 'Deal 6 damage.', effects: [{type: 'damage', value: 6}] },
                { id: 'defend', name: 'Defend', type: 'skill', cost: 1, rarity: 'starter', description: 'Gain 5 Block.', effects: [{type: 'block', value: 5}] },
                { id: 'bash', name: 'Bash', type: 'attack', cost: 2, rarity: 'starter', description: 'Deal 8 damage. Apply 2 Vulnerable.', effects: [{type: 'damage', value: 8}, {type: 'vulnerable', value: 2}] },
                { id: 'thermal_vent', name: 'Thermal Vent', type: 'skill', cost: 1, rarity: 'starter', description: 'Gain 4 Block. Gain 1 Overheat.', effects: [{type: 'block', value: 4}, {type: 'overheat', value: 1}] },
                { id: 'rage_spike', name: 'Rage Spike', type: 'attack', cost: 1, rarity: 'starter', description: 'Deal 4 damage. If you took damage this turn, deal 8 instead.', effects: [{type: 'damage', value: 4}] },
                { id: 'system_check', name: 'System Check', type: 'skill', cost: 0, rarity: 'starter', description: 'Draw 1 card. Exhaust.', effects: [{type: 'draw', value: 1}], exhaust: true },
                // Lyria starter fallbacks
                { id: 'lyria_strike', name: 'Fracture', type: 'attack', cost: 1, rarity: 'starter', description: 'Deal 5 damage.', effects: [{type: 'damage', value: 5}] },
                { id: 'lyria_defend', name: 'Foresight', type: 'skill', cost: 1, rarity: 'starter', description: 'Gain 5 Block.', effects: [{type: 'block', value: 5}] },
                { id: 'temporal_slip', name: 'Temporal Slip', type: 'skill', cost: 1, rarity: 'starter', description: 'Draw 2 cards. Discard 1. Gain 1 Temporal Flux.', effects: [{type: 'draw', value: 2}, {type: 'discard', value: 1}, {type: 'temporal_flux', value: 1}] },
                { id: 'astral_lance', name: 'Astral Lance', type: 'attack', cost: 2, rarity: 'starter', description: 'Deal 6 damage + Astral Charge. Gain 1 Astral Charge.', effects: [{type: 'damage', value: 6, scaling: 'astral_charge'}, {type: 'astral_charge', value: 1}] }
            ]
        };
        ...
        this.dataPaths = {
            ...
            events: {
                ...
                varra: 'events/varra_events.json',
                act1_bookends: 'events/act1_bookends.json'
            },
    ...
    async preload(progressCallback = null) {
        ...
        console.log(`[DataLoader] Preload complete. Cached ${this.cache.size} files.`);
    }
     ...
     * FIXED: Accepts both (cardId) and (heroId, cardId) signatures
     * GameState.initializeRun calls with (heroId, cardId)
     * @param {string} heroIdOrCardId - Either a heroId (if 2 args) or cardId (if 1 arg)
     * @param {string} [maybeCardId] - The actual cardId when called with 2 args
     * @returns {Object|null}
     */
    getCard(heroIdOrCardId, maybeCardId) {
        let cardId, heroId;
        
        if (maybeCardId !== undefined) {
            // Called as getCard(heroId, cardId)
            heroId = heroIdOrCardId;
            cardId = maybeCardId;
        } else {
            // Called as getCard(cardId)
            cardId = heroIdOrCardId;
            heroId = null;
        }
        
        // If heroId provided, search hero-specific cards first
        if (heroId) {
            const heroCardsPath = this.basePath + (this.dataPaths.cards[heroId] || '');
            const heroData = this.cache.get(heroCardsPath);
            if (heroData?.cards) {
                const card = heroData.cards.find(c => c.id === cardId);
                if (card) return { ...card };
            }
        }
        ...
        // Check fallback data
        if (this.fallbackData.cards) {
            const card = this.fallbackData.cards.find(c => c.id === cardId);
            if (card) return { ...card };
        }
    ...
    /**
     * Get all cards for a hero (hero-specific cards only)
     * @param {string} heroId 
     * @returns {Array}
     */
    getHeroCards(heroId) {
    ...
    /**
     * Get all cards available for a hero (hero-specific + neutral)
     * @param {string} heroId 
     * @returns {Array}
     */
    getCardsForHero(heroId) {
        const cards = [];
        
        // Get hero-specific cards
        const heroCards = this.getHeroCards(heroId);
        if (heroCards && heroCards.length > 0) {
            cards.push(...heroCards);
        }
        
        // Get neutral cards
        const neutralPath = this.basePath + this.dataPaths.cards.neutral;
        const neutralData = this.cache.get(neutralPath);
        if (neutralData?.cards) {
            cards.push(...neutralData.cards);
        }
        
        // If still empty, use fallback
        if (cards.length === 0) {
            console.warn('[DataLoader] No cards found, using fallback');
            return this.fallbackData.cards || [];
        }
    ...
    /**
     * Get the starter deck for a hero as full card objects
     * Converts card IDs from hero.startingDeck into full card objects
     * @param {string} heroId 
     * @returns {Array} Array of card objects with instanceIds
     */
    getStarterDeck(heroId) {
        console.log(`[DataLoader] Building starter deck for hero: ${heroId}`);
        
        // Get hero data
        const hero = this.getHero(heroId);
        if (!hero) {
            console.error(`[DataLoader] Hero not found: ${heroId}`);
            return this.getFallbackStarterDeck();
        }
        
        // Get the starting deck card IDs
        const startingDeckIds = hero.startingDeck;
        if (!startingDeckIds || startingDeckIds.length === 0) {
            console.warn(`[DataLoader] No startingDeck defined for hero: ${heroId}`);
            return this.getFallbackStarterDeck();
        }
        
        console.log(`[DataLoader] Starting deck IDs:`, startingDeckIds);
        
        // Get all available cards for this hero
        const allCards = this.getCardsForHero(heroId);
        console.log(`[DataLoader] Available cards pool: ${allCards.length} cards`);
        
        // Build the deck by looking up each card ID
        const deck = [];
        const timestamp = Date.now();
        
        startingDeckIds.forEach((cardId, index) => {
            const cardData = allCards.find(c => c.id === cardId);
            
            if (cardData) {
                // Create a card instance with unique instanceId
                const cardInstance = {
                    ...cardData,
                    instanceId: `${cardId}_${timestamp}_${index}`
                };
                
                // Normalize card properties for combat system
                // Convert effects array to direct properties for easier access
                if (cardData.effects && Array.isArray(cardData.effects)) {
                    cardData.effects.forEach(effect => {
                        if (effect.type === 'damage' && typeof effect.value === 'number') {
                            cardInstance.damage = effect.value;
                        }
                        if (effect.type === 'block' && typeof effect.value === 'number') {
                            cardInstance.block = effect.value;
                        }
                        if (effect.type === 'draw' && typeof effect.value === 'number') {
                            cardInstance.draw = effect.value;
                        }
                        if (effect.type === 'energy' && typeof effect.value === 'number') {
                            cardInstance.energy = effect.value;
                        }
                        if (effect.type === 'self_damage' && typeof effect.value === 'number') {
                            cardInstance.selfDamage = effect.value;
                        }
                        if (effect.type === 'vulnerable' && typeof effect.value === 'number') {
                            cardInstance.applyVulnerable = effect.value;
                        }
                        if (effect.type === 'weak' && typeof effect.value === 'number') {
                            cardInstance.applyWeak = effect.value;
                        }
                    });
                }
                
                deck.push(cardInstance);
                console.log(`[DataLoader] Added card: ${cardData.name} (${cardId})`);
            } else {
                console.warn(`[DataLoader] Card not found: ${cardId}, using fallback`);
                // Try to find in fallback data
                const fallbackCard = this.fallbackData.cards?.find(c => c.id === cardId);
                if (fallbackCard) {
                    deck.push({
                        ...fallbackCard,
                        instanceId: `${cardId}_${timestamp}_${index}`,
                        damage: fallbackCard.effects?.find(e => e.type === 'damage')?.value,
                        block: fallbackCard.effects?.find(e => e.type === 'block')?.value
                    });
                } else {
                    // Add a generic Strike as last resort
                    deck.push({
                        id: cardId,
                        instanceId: `${cardId}_${timestamp}_${index}`,
                        name: cardId.charAt(0).toUpperCase() + cardId.slice(1).replace(/_/g, ' '),
                        type: 'attack',
                        cost: 1,
                        damage: 6,
                        rarity: 'starter',
        ...
        console.log(`[DataLoader] Built starter deck with ${deck.length} cards`);
        return deck;
    }

    /**
     * Fallback starter deck if hero data is unavailable
     * @returns {Array}
     */
    getFallbackStarterDeck() {
        console.log('[DataLoader] Using fallback starter deck');
        const timestamp = Date.now();
        const deck = [];
    ...
    /**
     * Get card reward pool filtered by hero and rarity
     * Used by RewardSystem for generating combat rewards
     * @param {string} heroId - Hero identifier
     * @param {string} rarity - Card rarity filter
     * @returns {Array} Filtered card pool
     */
    getCardRewardPool(heroId, rarity) {
        const cards = [];
        
        // Get hero-specific cards
        const heroCards = this.getHeroCards(heroId);
        if (heroCards && heroCards.length > 0) {
            cards.push(...heroCards.filter(c => c.rarity === rarity));
        }
        ...
        const neutralData = this.cache.get(neutralPath);
        if (neutralData?.cards) {
            cards.push(...neutralData.cards.filter(c => c.rarity === rarity));
        }
        
        // Filter out starter cards from reward pool (they shouldn't appear as rewards)
        return cards.filter(c => c.rarity !== 'starter');
    }

    /**
     * Check if a card rarity is valid
     * @param {string} rarity 
     * @returns {boolean}
     */
    isValidRarity(rarity) {
        const validRarities = ['starter', 'common', 'uncommon', 'rare', 'legendary', 'corrupted'];
    ...
    getArtifact(artifactId) {
        // Search through all loaded artifact pools
        for (const [path, data] of this.cache) {
            if (path.includes('/artifacts/') || path.includes('relics')) {
                const artifacts = data.artifacts || data.relics || data;
        ...
        // Check fallback data
        if (this.fallbackData.artifacts) {
            const artifact = this.fallbackData.artifacts.find(a => a.id === artifactId);
            if (artifact) return { ...artifact };
        }
    ...
    getEnemy(enemyId) {
        ...
        // Check fallback
        if (this.fallbackData.enemies) {
            const enemy = this.fallbackData.enemies.find(e => e.id === enemyId);
            if (enemy) return { ...enemy };
        }
    ...
    /**
     * Get a random encounter for an act
     * @param {number} act - Act number (1, 2, or 3)
     * @param {string} difficulty - 'normal', 'elite', or 'boss'
     * @returns {Array} Array of enemy objects
     */
    getRandomEncounter(act, difficulty = 'normal') {
        console.log(`[DataLoader] Getting ${difficulty} encounter for Act ${act}`);
        
        if (difficulty === 'boss') {
            const boss = this.getBossForAct(act);
            return boss ? [boss] : this.getEnemiesForAct(act, 'elite');
        }
        
        return this.getEnemiesForAct(act, difficulty);
    }
    ...
    getEnemiesForAct(act, difficulty = 'normal') {
        ...
        for (let i = 0; i < count && pool.length > 0; i++) {
            ...
            if (enemy.intents && enemy.intents.length > 0) {
                ...
                enemy.intent = { type: 'attack', damage: 6 };
            }
    ...
    getBossForAct(act) {
        ...
        let boss = bosses.find(b => b.id === bossId) || bosses[0];
        
        if (boss) {
            boss = { ...boss };
            boss.currentHp = boss.hp || boss.maxHp || 80;
            boss.maxHp = boss.maxHp || boss.hp || 80;
            boss.block = 0;
            
            if (boss.intents && boss.intents.length > 0) {
                boss.intent = { ...boss.intents[0] };
            }
        }
     ...
     * @param {string} heroId - Optional hero ID for hero-specific events
     * @param {EventManager} eventManager - Optional EventManager for v2 selection (history, flags, chains)
     * @returns {Object}
     */
    getRandomEvent(act, corruption = 0, heroId = null, eventManager = null) {
        ...
        // v2: Delegate to EventManager for smart selection (history, flags, chains)
        if (eventManager) {
            return eventManager.selectEvent(data, act);
        }
        
        // Legacy fallback: Filter events by act and corruption requirements
        let pool = data.filter(e => {
    ...
    /**
     * Get a random artifact/relic by rarity
     * @param {string} rarity - 'common', 'uncommon', 'rare', 'legendary', or 'mythic'
     * @returns {Object}
     */
    getRandomArtifact(rarity = 'common') {
        let allArtifacts = [];
        
        // Gather artifacts from all cached sources
        for (const [path, data] of this.cache) {
            if (path.includes('/artifacts/') || path.includes('relics')) {
                const artifacts = data.artifacts || data.relics || [];
                if (Array.isArray(artifacts)) {
        ...
        if (allArtifacts.length === 0) {
            allArtifacts = this.fallbackData.artifacts || [];
        }
        
        if (allArtifacts.length === 0) return null;
        
        // Filter by rarity
        let pool = allArtifacts.filter(a => a.rarity === rarity);
        
        // If no artifacts of requested rarity, fall back to any
        if (pool.length === 0) pool = allArtifacts;
        
        return { ...pool[Math.floor(Math.random() * pool.length)] };
    }

    /**
     * Get all artifacts/relics of a specific rarity
     * @param {string} rarity 
     * @returns {Array}
     */
