 ...
 * 
 * @version 0.6.0 MVP - Feature Pack v4: Void Systems (Fragments, Chain Multiplier, Meta Corruption)
 */
...
// Feature Pack v4: Void Systems (Balatro Jokers + Chain Multiplier + Inscryption Meta Corruption)
import { VoidSystemsIntegration } from './systems/VoidSystemsIntegration.js';

// Fragment Reward UI (Void Fragment acquisition overlay)
import { showFragmentReward, shouldOfferFragment } from './ui/FragmentRewardOverlay.js';
...
class ShatteredStar {
    constructor() {
        ...
        // Feature Pack v4: Void Systems (Fragments + Chains + Meta Corruption)
        this.voidSystems = null;
    ...
    async init() {
        ...
        try {
            ...
            // CRITICAL FIX: Initialize SaveManager to register auto-save event listeners
            try {
                this.saveManager.init();
                console.log('[Shattered Star] SaveManager initialized');
            } catch (e) {
                console.warn('[Shattered Star] SaveManager init failed (non-fatal):', e);
            }
    ...
    initializeSystems() {
        ...
        // Initialize Feature Pack v4 (Void Fragments, Chain Multiplier, Meta Corruption)
        try {
            this.voidSystems = new VoidSystemsIntegration(this);
            console.log('[Shattered Star] Void systems (Fragments + Chains + Meta Corruption) initialized');
        } catch (e) {
            console.warn('[Shattered Star] Void systems init failed (non-fatal):', e);
        }
    ...
    setupGlobalEvents() {
        ...
        // ── Fallback handlers for card/artifact events from EventScreen ──
        // These ensure cards and artifacts are properly added even if the
        // event source didn't handle it directly
        
        this.eventBus.on('card:obtained', (cardData) => {
            // If cardData already has instanceId, EventScreen handled it
            if (cardData?.instanceId) return;
            
            // If we only got an id, try to look up and add to deck
            if (cardData?.id && !cardData.name) {
                console.log(`[Shattered Star] Fallback card:obtained handler for: ${cardData.id}`);
                const heroId = this.state.get('hero.id') || 'korvax';
                let fullCard = null;
                try {
                    fullCard = this.dataLoader?.getCard?.(heroId, cardData.id) 
                            || this.dataLoader?.getCard?.(cardData.id);
                } catch (e) {
                    console.warn('[Shattered Star] Fallback card lookup failed:', e);
                }
                
                if (fullCard) {
                    fullCard.instanceId = `${fullCard.id}_fallback_${Date.now()}`;
                    const deck = this.state.get('deck') || [];
                    deck.push(fullCard);
                    this.state.set('deck', deck);
                    console.log(`[Shattered Star] Fallback added card to deck: ${fullCard.name}`);
                }
            }
        });
        
        this.eventBus.on('card:upgrade', (data) => {
            // Fallback: if EventScreen didn't show upgrade UI, try auto-upgrade
            console.log('[Shattered Star] Fallback card:upgrade handler:', data);
            const deck = this.state.get('deck') || [];
            const upgradeable = deck.filter(c => !c.upgraded);
            
            if (upgradeable.length > 0) {
                const target = (data?.target && typeof data.target === 'string' && data.target !== 'random' && data.target !== 'choose')
                    ? upgradeable.find(c => c.id === data.target) || upgradeable[0]
                    : upgradeable[Math.floor(Math.random() * upgradeable.length)];
                
                if (target && this.cardUpgradeSystem) {
                    this.cardUpgradeSystem.upgradeCard(target);
                    this.state.set('deck', deck);
                    console.log(`[Shattered Star] Fallback upgraded: ${target.name}`);
                    this.eventBus.emit('card:upgraded', target);
                }
            }
        });
        ...
        if (btnGameoverMenu) {
            btnGameoverMenu.addEventListener('click', () => {
                console.log('[Shattered Star] Game Over - Menu clicked');
                this.screenManager.transitionTo('start-screen');
            });
        }
        
        // CRITICAL FIX: Victory Continue button handler
        const btnContinueRun = document.getElementById('btn-continue-run');
        if (btnContinueRun) {
            btnContinueRun.addEventListener('click', () => {
                console.log('[Shattered Star] Victory - Continue clicked');
                const currentAct = this.state.get('act') || 1;
                if (currentAct >= 3) {
                    // Final act complete - show full victory
                    this.handleVictory();
                } else {
                    // Advance to next act
                    this.state.set('act', currentAct + 1);
                    this.state.set('actComplete', false);
                    this.mapGenerator.generateAct(currentAct + 1);
    ...
    startNewRun(heroId) {
        ...
        // Reset void systems for new run
        if (this.voidSystems) {
            try {
                this.voidSystems.fragments.equippedFragments = [];
                this.voidSystems.fragments.collectedFragments = [];
                this.voidSystems.chains.resetChain();
            } catch (e) {
                console.warn('[Shattered Star] Void systems reset failed (non-fatal):', e);
            }
        }
    ...
    handleCombatVictory() {
        ...
        // Check if we should offer a void fragment reward
        if (shouldOfferFragment(this, nodeType)) {
            const pool = nodeType === 'elite' ? 'common,uncommon' : 'all';
            showFragmentReward(this, {
                pool,
                count: 3,
                source: `combat_${nodeType}`,
                allowSkip: true,
                onComplete: () => {
    ...
    handleBossVictory() {
        ...
        if (act >= 3) {
            ...
            // Always offer a fragment after boss (rare pool)
            if (shouldOfferFragment(this, 'boss')) {
                showFragmentReward(this, {
                    pool: 'uncommon,rare',
                    count: 3,
                    source: `boss_act${act}`,
