 ...
 * 
 * @version 0.4.0 MVP - Feature Pack: Reward Shimmer, Void Merchant, Boss Narrative, Varra NPC
 */
...
class ShatteredStar {
    ...
    setupGlobalEvents() {
        ...
        // Handle visibility change — session end hook with preview
        document.addEventListener('visibilitychange', () => {
            if (document.hidden && this.state.get('runActive')) {
                try {
                    // Show "what's ahead" preview to create return motivation
                    this.showSessionEndPreview();
                    
                    // Serialize CorruptionThresholds state
                    if (this.corruptionThresholds) {
                        this.state.set('corruptionThresholds', this.corruptionThresholds.serialize(), true);
                    }
    ...
    /**
     * Show a "what's ahead" preview when the player tabs away
     * Creates psychological return motivation (session end hook)
     */
    showSessionEndPreview() {
        try {
            // Only show on map screen or between encounters
            const currentScreen = this.screenManager?.currentScreen;
            if (currentScreen === 'title-screen' || currentScreen === 'start-screen') return;
            
            const corruption = this.state.get('corruption') || 0;
            const act = this.state.get('act') || 1;
            const floor = this.state.get('floor') || 1;
            
            // Figure out what's next based on map state
            const mapState = this.state.get('mapState');
            let nextNodeType = null;
            let teaser = '';
            
            if (mapState && mapState.nodes) {
                // Find the next unvisited node
                const currentRow = this.state.get('currentRow') || 0;
                const nextNodes = mapState.nodes.filter(n => n.row === currentRow + 1 && !n.visited);
                if (nextNodes.length > 0) {
                    // Pick the first available next node
                    nextNodeType = nextNodes[0].type;
                }
            }
            
            // Generate teaser based on next node or general state
            const teasers = {
                'combat': [
                    'Hostiles detected on the next deck...',
                    'Motion sensors are going wild ahead...',
                    'Something is waiting in the dark...'
                ],
                'elite': [
                    'A powerful presence bars your path...',
                    'Elite hostiles ahead. Prepare yourself.',
                    'The next fight won\'t be easy...'
                ],
                'event': [
                    'A distress signal flickers from ahead...',
                    'Someone — or something — wants to talk...',
                    'A choice awaits in the next corridor...'
                ],
                'shop': [
                    'A merchant\'s neon sign glows ahead...',
                    'Time to spend those credits wisely...',
                    'The market stalls hum with possibility...'
                ],
                'rest': [
                    'A safe room ahead. You could use the rest.',
                    'A moment of peace awaits...',
                    'Shelter. Heal. Then push on.'
                ],
                'boss': [
                    'The boss chamber looms ahead...',
                    'This is it. The final challenge of the act.',
                    'Everything has led to this...'
                ]
            };
            
            if (nextNodeType && teasers[nextNodeType]) {
                const pool = teasers[nextNodeType];
                teaser = pool[Math.floor(Math.random() * pool.length)];
            } else {
                teaser = `Act ${act}, Floor ${floor}. The station grows darker...`;
            }
            
            // Add corruption warning if high
            let corruptionNote = '';
            if (corruption >= 75) {
                corruptionNote = `<div style="color: #ff0040; margin-top: 8px; font-size: 0.75rem;">
                    ⚠ Corruption: ${corruption}% — The void tightens its grip</div>`;
            } else if (corruption >= 50) {
                corruptionNote = `<div style="color: #bf00ff; margin-top: 8px; font-size: 0.75rem;">
                    Corruption: ${corruption}% — Reality thins around you</div>`;
            }
            
            // Create subtle overlay (won't block since page is hidden)
            const existing = document.getElementById('session-end-preview');
            if (existing) existing.remove();
            
            const overlay = document.createElement('div');
            overlay.id = 'session-end-preview';
            overlay.style.cssText = `
                position: fixed; bottom: 0; left: 0; width: 100%;
                background: linear-gradient(transparent, rgba(0, 0, 0, 0.95));
                padding: 30px 20px 20px; z-index: 3000;
                text-align: center; font-family: 'Courier New', monospace;
            `;
            
            overlay.innerHTML = `
                <div style="font-size: 0.65rem; letter-spacing: 0.3em; color: #00c8ff;
                    text-transform: uppercase; margin-bottom: 6px;">
                    Progress Saved
                </div>
                <div style="color: #c0c0d0; font-size: 0.9rem; font-style: italic;">
                    ${teaser}
                </div>
                ${corruptionNote}
            `;
            ...
            // Auto-remove after 3 seconds (in case they come back quickly)
            setTimeout(() => {
                if (overlay.parentNode) overlay.remove();
            }, 3000);
            
            console.log(`[Shattered Star] Session end preview shown: "${teaser}"`);
        } catch (e) {
            // Non-critical, don't let this break saving
            console.warn('[Shattered Star] Session end preview failed:', e);
        }
    }
