...
export function setupCombatScreen(game) {
    ...
    function renderPlayerStatuses() {
        ...
        if (!container) {
            ...
            container.style.cssText = `
                position: absolute; bottom: 155px; left: 10px;
                display: flex; flex-wrap: wrap; gap: 4px; z-index: 60;
                max-width: 300px;
            `;
        ...
        const statusIcons = {
            strength: { icon: '💪', color: '#ff4444', label: 'Strength' },
            dexterity: { icon: '🛡️', color: '#44ff44', label: 'Dexterity' },
            vulnerable: { icon: '🎯', color: '#ff6600', label: 'Vulnerable' },
            weak: { icon: '💫', color: '#888888', label: 'Weak' },
            frail: { icon: '💔', color: '#9966cc', label: 'Frail' },
            poison: { icon: '☠️', color: '#00cc00', label: 'Poison' },
            bleed: { icon: '🩸', color: '#cc0000', label: 'Bleed' },
            burn: { icon: '🔥', color: '#ff4400', label: 'Burn' },
            regen: { icon: '💚', color: '#00ff88', label: 'Regen' },
        };
        ...
        // Persistent buffs from combat state
        if (strength > 0) {
            html += renderStatusBadge(statusIcons.strength, strength);
        }
        if (dexterity > 0) {
            html += renderStatusBadge(statusIcons.dexterity, dexterity);
        }
        
        // Duration/intensity statuses from combat.status.*
        Object.keys(statusIcons).forEach(id => {
            if (id === 'strength' || id === 'dexterity') return; // already handled
            const val = getPlayerStatus(id);
            if (val > 0) {
    ...
    function renderStatusBadge(info, value) {
        return `
            <div class="player-status-badge" title="${info.label}: ${value}" style="
                background: rgba(0,0,0,0.7); border: 1px solid ${info.color};
                border-radius: 4px; padding: 2px 6px; font-size: 0.75rem;
                color: ${info.color}; display: flex; align-items: center; gap: 3px;
                text-shadow: 0 0 4px ${info.color}40;
            ">
                <span>${info.icon}</span><span style="font-weight:bold;">${value}</span>
            </div>
        `;
    }
        ...
        container.innerHTML = enemies.map((enemy, i) => {
            ...
            return `
                <div class="enemy enemy-${enemyType}" 
                    ...
                    ${enemy.poison > 0 ? `<div class="enemy-debuff poison" style="color:#00cc00;">☠️ ${enemy.poison}</div>` : ''}
                    ${enemy.bleed > 0 ? `<div class="enemy-debuff bleed" style="color:#cc0000;">🩸 ${enemy.bleed}</div>` : ''}
                    ${enemy.burn > 0 ? `<div class="enemy-debuff burn" style="color:#ff4400;">🔥 ${enemy.burn}</div>` : ''}
                </div>
            `;
        }).join('');
    ...
    function renderEnergy() {
        ...
        // ── Lyria Resource Display ──
        const heroId = game.state.get('hero.id');
        if (heroId === 'lyria') {
            const astral = game.state.get('combat.astralCharge') || 0;
            const flux = game.state.get('combat.temporalFlux') || 0;
            let resDisplay = document.getElementById('lyria-resources');
            if (!resDisplay) {
                resDisplay = document.createElement('div');
                resDisplay.id = 'lyria-resources';
                resDisplay.style.cssText = 'position:absolute; top:60px; left:10px; font-size:0.8rem; color:#a855f7; z-index:50; text-shadow:0 0 4px rgba(168,85,247,0.5);';
                const combatScreen = document.getElementById('combat-screen');
                if (combatScreen) combatScreen.appendChild(resDisplay);
            }
            resDisplay.innerHTML = `✨ Astral: ${astral} &nbsp; ⏳ Flux: ${flux}`;
        }
    }
    ...
    function renderRelicBar() {
        ...
        if (!container) {
            ...
            container.style.cssText = `
                ...
                display: flex; gap: 4px; z-index: 60; flex-wrap: wrap;
    ...
    function applyCardEffects(card, targetIndex) {
        ...
        if (card.damage && card.damage > 0) {
            if (targetIndex !== null && targetIndex >= 0 && enemies[targetIndex]) {
                let damage = calculatePlayerDamage(card.damage);
    ...
    function applyEnemyDebuffToPlayer(effect, value) {
        ...
        switch (effect) {
            ...
            case 'corruption':
                ...
                game.state.set('corruption', Math.min(100, corruption + corruptionGain));
