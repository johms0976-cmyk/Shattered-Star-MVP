 ...
 * @version 0.6.0 — Card animations, Lyria mechanics (Astral Charge, Temporal Flux), upgrade flavor text
 */
...
export function setupCombatScreen(game) {
            ...
            card.effects.forEach(effect => {
                switch (effect.type) {
                    case 'damage':
                        ...
                        if (typeof effect.value === 'number' && normalized.damage === undefined) normalized.damage = effect.value;
                        break;
                    case 'block':
                    case 'block_gain':
                        if (typeof effect.value === 'number' && normalized.block === undefined) normalized.block = effect.value;
                        break;
                    case 'draw':
                    ...
                    case 'trigger':
                        ...
                        break;
                    // Lyria/hero-specific types handled in applyCardEffects
                    case 'astral_charge':
                    case 'temporal_flux':
                    case 'consume_astral':
                        break; // preserved in effects array
                    default:
    ...
    function renderHeroResources() {
        ...
        if (!container) {
            ...
            container.className = 'hero-resource-hud';
            const combatScreen = document.getElementById('combat-screen');
            if (combatScreen) combatScreen.appendChild(container);
        }
    ...
    function applyCardEffects(card, targetIndex) {
        ...
        if (card.damage && card.damage > 0) {
            if (targetIndex !== null && targetIndex >= 0 && enemies[targetIndex]) {
                // Determine base damage — support overheat-scaling cards
                let cardDamage = card.damage;
                if (card.damage === 'overheat' || card.damageMultiplier === 'overheat') {
                ...
                if (hasAstralScaling) {
                    ...
                    if (astralCharges > 0) {
                        console.log(`[CombatScreen] Astral scaling: +${astralCharges} damage`);
                    }
                }
                
                // ── Biome damage modifier (Purifying Light / Xal'Korath's Gaze) ──
                if (biomeEffects) {
                ...
                let voidChainResult = null;
                if (game.voidSystems) {
                    try {
                        const fragDmg = game.voidSystems.fragments.calculateDamageModifiers(card, damage, enemies[targetIndex]);
                        if (fragDmg.flatBonus !== 0 || fragDmg.multiplier !== 1.0) {
                            damage += fragDmg.flatBonus;
                            damage = Math.floor(damage * fragDmg.multiplier);
                            console.log(`[CombatScreen] Void fragment damage: +${fragDmg.flatBonus} flat, x${fragDmg.multiplier} mult`);
                        }
                        // Apply chain multiplier
                        const chainedDamage = game.voidSystems.chains.applyToValue(damage, false);
                        if (chainedDamage !== damage) {
                            console.log(`[CombatScreen] Void chain: ${damage} → ${chainedDamage} (x${game.voidSystems.chains.getCurrentMultiplier().toFixed(1)})`);
                ...
                // ── Balatro-style damage cascade visualization ──
                if (damageCascade) {
                    try {
                        const modifiers = {
                            strength: game.state.get('combat.strength') || 0,
                            rage: game.state.get('combat.rage') || 0,
                            overheatBonus: (game.state.get('combat.overheat') || 0) >= 5 ? 2 : 0,
                            astralCharge: hasAstralScaling ? (game.state.get('combat.astralCharge') || 0) : 0,
                            relicBonus: getRelicBonus('strength'),
                            biomeMultiplier: biomeEffects ? biomeEffects.getDamageModifier(card) : 1.0,
                            empowerBonus: corruptionCurrency ? corruptionCurrency.getEmpowerBonus() : 0,
                            weak: getPlayerStatus('weak') > 0,
                            vulnerable: enemies[targetIndex]?.vulnerable > 0
                        };
                        const targetEl = document.querySelector(`.enemy[data-index="${targetIndex}"]`);
                        const { steps, finalDamage } = damageCascade.buildDamageSteps(cardDamage, modifiers);
                        damageCascade.showDamageCascade(targetEl, steps, finalDamage, { skipIfSimple: true });
                    } catch (e) {
                        console.warn('[CombatScreen] Damage cascade visual failed (non-fatal):', e);
                    }
                }
                ...
                for (let h = 0; h < hits; h++) {
                    if (!enemies[targetIndex] || enemies[targetIndex].currentHp <= 0) break;
                    ...
                    enemies[targetIndex].currentHp -= hpDamage;
                    totalDealt += hpDamage;
                    ...
                    if (hits > 1) {
                        console.log(`[CombatScreen] Hit ${h+1}/${hits}: ${damage} damage to ${enemies[targetIndex].name} (${blocked} blocked)`);
                    }
                }
                
                console.log(`[CombatScreen] Dealt ${totalDealt + totalBlocked} total damage to ${enemies[targetIndex].name} (${totalBlocked} blocked), HP: ${enemies[targetIndex].currentHp}/${enemies[targetIndex].maxHp}`);
                
                // Emit damage event for VFX
                game.eventBus.emit('damage:dealt', { target: enemies[targetIndex], amount: totalDealt + totalBlocked, blocked: totalBlocked, hits });
                
                // Check for enemy death
                if (enemies[targetIndex].currentHp <= 0) {
                    console.log(`[CombatScreen] ☠ ${enemies[targetIndex].name} defeated!`);
                    
                    // ── VOID SYSTEMS: Process enemy kill (Lethal Siphon, Overkill Cascade, Death Rattle) ──
                    if (game.voidSystems) {
                        try {
                            ...
                            console.warn('[CombatScreen] Void systems enemy kill failed (non-fatal):', e);
                        }
                    }
        ...
        if (card.block && card.block > 0) {
            let block = calculatePlayerBlock(card.block);
    ...
    function triggerMeltdown(enemies) {
        ...
        game.state.set('hp', Math.max(1, hp - selfDamage));
        ...
        enemies.forEach((enemy, i) => {
            ...
            function applyEnemyHit(baseDamage) {
                ...
                if (hpDamage > 0) {
                    ...
                    const heroId = game.state.get('heroId') || game.state.get('hero.id') || 'korvax';
                    const triggers = game.state.get('combat.triggers') || [];
                    const rageTrigger = triggers.find(t => t.on === 'take_damage' && t.apply?.type === 'rage');
                    if (rageTrigger) {
                        const currentRage = game.state.get('combat.rage') || 0;
                        const rageGain = rageTrigger.apply.value || 1;
                        game.state.set('combat.rage', currentRage + rageGain);
                        console.log(`[CombatScreen] Rage triggered: +${rageGain} (total: ${currentRage + rageGain})`);
                    }
                }
    ...
    function applyEnemyDebuffToPlayer(effect, value) {
        ...
        switch (effect) {
            ...
            case 'corruption':
                ...
                game.eventBus.emit('corruption:gained', corruptionGain);
