 ...
 * FIXED: Verifies combat is actually active and starts it if not
 */
function initCombatUI(game) {
    console.log('[CombatScreen] ====== INIT COMBAT UI ======');
    
    if (!game.combat) {
        console.error('[CombatScreen] CRITICAL: Combat system not available!');
        return;
    }
    
    // CRITICAL FIX: Check if combat is actually active
    console.log('[CombatScreen] Combat state check:', {
        inCombat: game.combat.inCombat,
        enemyCount: game.combat.enemies?.length || 0,
        phase: game.combat.phase
    });
    
    // If combat isn't active, we need to start it
    if (!game.combat.inCombat || !game.combat.enemies || game.combat.enemies.length === 0) {
        console.warn('[CombatScreen] Combat not active! Attempting to start...');
        
        // Check for pending combat in state
        const pendingCombat = game.state.get('pendingCombat');
        if (pendingCombat && pendingCombat.enemies && pendingCombat.enemies.length > 0) {
            console.log('[CombatScreen] Found pending combat, starting:', pendingCombat.enemies);
            try {
                game.combat.startCombat(pendingCombat.enemies);
                game.state.set('pendingCombat', null);
            } catch (e) {
                console.error('[CombatScreen] Failed to start pending combat:', e);
            }
        } else {
            // Emergency fallback - create a basic enemy
            console.warn('[CombatScreen] No pending combat found, using emergency fallback!');
            const act = game.state.get('act') || 1;
            const emergencyEnemies = [{ 
                id: 'rustborn_raider', 
            ...
            try {
                game.combat.startCombat(emergencyEnemies);
            } catch (e) {
                console.error('[CombatScreen] Failed to start emergency combat:', e);
            }
        }
    }
    ...
    if (!game.combat.inCombat) {
        console.error('[CombatScreen] CRITICAL: Combat still not active after initialization attempt!');
        // Show error state in UI
        const enemyArea = document.getElementById('enemy-area');
        if (enemyArea) {
            enemyArea.innerHTML = '<div class="combat-error">Combat failed to initialize. Please return to map.</div>';
        }
        return;
    }
    
    console.log('[CombatScreen] Combat verified active, updating UI...');
    console.log('[CombatScreen] Enemies:', game.combat.enemies?.map(e => e.name));
    ...
    console.log('[CombatScreen] ====== INIT COMBAT UI COMPLETE ======');
}
