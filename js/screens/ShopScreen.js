    ...
    function generateShop() {
        ...
        shopInventory = {
            ...
            services: [
                ...
                { id: 'upgrade_card', name: 'Upgrade Card', cost: 50, type: 'service' }
            ]
        };
        
        // Generate 3-5 cards for sale with legendary chance
        const cardCount = 3 + Math.floor(Math.random() * 3);
        for (let i = 0; i < cardCount; i++) {
            // Rarity roll with legendary chance (only in Act 2+)
            ...
            if (roll < 50) {
                ...
                rarity = 'legendary';  // 2% legendary chance in shop
            }
            
            // Only allow legendary in Act 2+
            if (rarity === 'legendary' && act < 2) {
                rarity = 'rare';
            }
            ...
            if (pool.length > 0) {
                ...
                // Avoid duplicates
                if (!shopInventory.cards.find(c => c.id === card.id)) {
        ...
        // Generate 1-2 artifacts with rarity based on act
        const artifactCount = 1 + Math.floor(Math.random() * 2);
        for (let i = 0; i < artifactCount; i++) {
            // Higher chance of better artifacts in later acts
            const artifactRoll = Math.random() * 100;
            let artifactRarity;
            if (act === 1) {
                artifactRarity = artifactRoll < 70 ? 'common' : 'uncommon';
            } else if (act === 2) {
                artifactRarity = artifactRoll < 40 ? 'common' : (artifactRoll < 85 ? 'uncommon' : 'rare');
            } else {
                artifactRarity = artifactRoll < 20 ? 'common' : (artifactRoll < 60 ? 'uncommon' : (artifactRoll < 95 ? 'rare' : 'legendary'));
            }
            
            const artifact = game.dataLoader.getRandomArtifact(artifactRarity);
            if (artifact && !shopInventory.artifacts.find(a => a.id === artifact.id)) {
