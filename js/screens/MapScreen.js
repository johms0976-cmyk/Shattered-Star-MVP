 ...
 * Fixed: Proper map generation handling, CSS class consistency, re-render after generation
 */
...
function createNodeElement(node, currentNodeId, visitedNodes, game) {
    ...
    if (isAvailable) {
        ...
        el.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log(`[MapScreen] Click detected on node: ${node.id} (${node.type})`);
        ...
        el.addEventListener('touchend', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log(`[MapScreen] Touch detected on node: ${node.id} (${node.type})`);
            handleNodeClick(game, node);
        });
    } else {
        el.style.cursor = 'default';
        el.style.pointerEvents = node.visited ? 'none' : 'auto';
    }
...
function renderConnections(game, container, map) {
    ...
    map.paths.forEach(path => {
        ...
        if (fromNode && toNode) {
            ...
            // Highlight paths from first layer if no current node
...
function handleNodeClick(game, node) {
    console.log(`[MapScreen] Node clicked: ${node.id} (${node.type})`);
    ...
    // Mark node as visited through MapGenerator
    try {
        const selected = game.mapGenerator.selectNode(node.id);
        if (!selected) {
            console.warn('[MapScreen] Node selection failed');
            return;
        }
    } catch (e) {
        console.error('[MapScreen] Error selecting node:', e);
        return;
    }
