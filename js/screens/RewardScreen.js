...
export function setupRewardScreen(game) {
    ...
    /**
     * Force screen transition (same pattern as MapScreen/CombatScreen)
     */
    function forceScreenTransition(game, targetScreenId) {
        console.log(`[RewardScreen] forceScreenTransition to: ${targetScreenId}`);
        
        const rewardScreen = document.getElementById('reward-screen');
        const targetScreen = document.getElementById(targetScreenId);
        
        if (!targetScreen) {
            console.error(`[RewardScreen] Target screen not found: ${targetScreenId}`);
            return;
        }
        
        // STEP 1: Immediately hide the reward screen
        if (rewardScreen) {
            console.log('[RewardScreen] Hiding reward-screen immediately');
            rewardScreen.classList.remove('active', 'fade-in');
            rewardScreen.classList.add('fade-out');
            rewardScreen.style.cssText = '';
        }
        ...
        try {
            if (game.screenManager) {
                ...
                console.log('[RewardScreen] ScreenManager.transitionTo called');
            }
        } catch (e) {
            console.error('[RewardScreen] screenManager.transitionTo failed:', e);
        }
        
        // STEP 3: ALWAYS force the transition after a delay
        setTimeout(() => {
            console.log(`[RewardScreen] Force-showing ${targetScreenId} (backup)`);
            ...
            game.eventBus.emit('screen:changed', { to: targetScreenId, from: 'reward-screen' });
            game.eventBus.emit('screen:show', targetScreenId);
            game.eventBus.emit('screen:change', targetScreenId);
            
            console.log(`[RewardScreen] Force transition complete.`);
        }, 100);
    }
