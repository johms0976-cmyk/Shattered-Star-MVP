 ...
 * FIXED: Block not working - block now persists through enemy turn, resets on next player turn
    ...
    function enemyTurn() {
        ...
        enemies.forEach((enemy, i) => {
            ...
            if (enemy.intent.type === 'attack') {
                let damage = Number(enemy.intent.damage) || 5;
                ...
                // NaN protection for player HP
                if (!Number.isFinite(playerHp)) {
                    console.warn(`[CombatScreen] NaN detected in player HP! Resetting.`);
                    playerHp = Math.max(0, (game.state.get('hp') || 50) - (Number(enemy.intent.damage) || 5));
                }
                
                console.log(`[CombatScreen] ${enemy.name} attacks for ${damage + blocked} (${blocked} blocked, ${damage} to HP)`);
                
                game.eventBus.emit('player:damaged', { amount: damage + blocked, blocked });
                ...
                console.log(`[CombatScreen] ${enemy.name} gains ${blockAmount} block`);
            }
