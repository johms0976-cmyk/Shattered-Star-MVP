 ...
 * FIXED: Cards now properly persist even if DataLoader.getCard fails
 * @version 0.2.1
 */
class RewardSystem {
     ...
     * FIXED: Now properly adds card to deck even if DataLoader.getCard fails
     */
    selectCardReward(cardId) {
        const card = this.currentRewards.cards.find(c => c.id === cardId);
        if (!card) {
            console.warn('[RewardSystem] Card not found in rewards:', cardId);
            return;
        }
        
        // Get hero ID and try to get full card data from DataLoader
        const heroId = this.state.get('hero.id');
        let fullCard = null;
        ...
        try {
            ...
            console.warn('[RewardSystem] DataLoader.getCard failed:', e);
        }
        
        // FIXED: Use the card from rewards if DataLoader fails
        // This ensures the card is ALWAYS added to the deck
        const cardToAdd = fullCard || card;
        
        // Create card with unique instance ID
        const newCard = { 
            ...cardToAdd, 
        ...
        console.log(`[RewardSystem] Card added to deck: ${newCard.name}. Deck now has ${deck.length} cards`);
        
        // Emit events
        this.eventBus.emit('card:added', newCard);
        this.eventBus.emit('reward:selected', { type: 'card', card: newCard });
    ...
    collectCredits() {
        ...
        console.log(`[RewardSystem] Credits collected: +${this.currentRewards.credits}. Total: ${currentCredits + this.currentRewards.credits}`);
    ...
    generateEventRewards(effects) {
        effects.forEach(effect => {
            switch (effect.type) {
                ...
                case 'addCard':
                    // Handle card addition from events
                    if (effect.card) {
                        const deck = this.state.get('deck') || [];
                        const newCard = {
                            ...effect.card,
                        ...
                        this.state.set('deck', deck);
                        this.eventBus.emit('card:added', newCard);
                        console.log(`[RewardSystem] Event card added: ${newCard.name}`);
                    }
                    break;
                ...
                case 'artifact':
                    ...
                    if (artifact) {
                        ...
                        console.log(`[RewardSystem] Artifact gained: ${artifact.name}`);
                    }
                    break;
