 ...
 * ADDED: "Commune with the Void" option to discover fragments
 * @version 0.3.0
 */
...
export function setupRestScreen(game) {
    ...
    function updateRestOptions() {
        ...
        // COMMUNE OPTION: Show "Commune with the Void" if VoidFragmentSystem is available
        let communeBtn = document.getElementById('btn-rest-commune');
        const hasFragmentSystem = !!game.voidSystems?.fragments;
        const corruption = game.state.get('corruption') || 0;
        const communeAvailable = hasFragmentSystem && corruption >= 10;
        
        if (communeAvailable) {
            // Create button if it doesn't exist
            if (!communeBtn) {
                const optionsContainer = document.querySelector('.rest-options') 
                    || document.getElementById('btn-rest-heal')?.parentElement;
                if (optionsContainer) {
                    communeBtn = document.createElement('button');
                    communeBtn.id = 'btn-rest-commune';
                    communeBtn.className = 'rest-option';
                    optionsContainer.appendChild(communeBtn);
                    console.log('[RestScreen] Created Commune button');
                }
            }
            if (communeBtn) {
                const communeCorruptionCost = 5;
                communeBtn.innerHTML = `
                    <span class="option-icon">👁️</span>
                    <span class="option-text">COMMUNE</span>
                    <span class="option-desc">Discover a Void Fragment (+${communeCorruptionCost} Corruption)</span>
                `;
                communeBtn.disabled = false;
                communeBtn.style.display = '';
                console.log('[RestScreen] Commune button updated');
            }
        } else if (communeBtn) {
            communeBtn.style.display = 'none';
        }
    }
    ...
    function setupButtons() {
        ...
        // Commune button (discover void fragment)
        const communeBtn = document.getElementById('btn-rest-commune');
        if (communeBtn) {
            const newCommuneBtn = communeBtn.cloneNode(true);
            communeBtn.parentNode.replaceChild(newCommuneBtn, communeBtn);
            
            newCommuneBtn.addEventListener('click', () => {
                console.log('[RestScreen] Commune clicked');
                game.audioManager?.playSFX?.('ui_click');
                
                // Apply corruption cost
                const communeCorruptionCost = 5;
                const corruption = game.state.get('corruption') || 0;
                const newCorruption = Math.min(100, corruption + communeCorruptionCost);
                game.state.set('corruption', newCorruption);
                game.eventBus.emit('corruption:changed', {
                    current: newCorruption,
                    previous: corruption,
                    delta: communeCorruptionCost
                });
                game.eventBus.emit('corruption:gained', communeCorruptionCost);
                
                // Show fragment reward overlay — complete rest when done
                showFragmentReward(game, {
                    pool: 'common,uncommon',
                    count: 3,
                    source: 'rest_commune',
                    allowSkip: false, // They paid corruption, they get the choice
                    onComplete: () => {
                        completeRest();
                    }
                });
    ...
    function upgradeCard(instanceId) {
        ...
        if (card && !card.upgraded) {
            ...
            if (game.cardUpgradeSystem?.upgradeCard) {
                ...
                console.log(`[RestScreen] Named upgrade: ${card.originalName || card.name} → ${card.name}`);
            } else {
