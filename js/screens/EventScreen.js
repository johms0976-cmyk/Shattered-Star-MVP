...
export function setupEventScreen(game) {
    ...
    function generateRandomEvent() {
        ...
        try {
            // Try to get event from data loader
            const event = game.dataLoader?.getRandomEvent?.(act, corruption, hero);
    ...
    function processEffects(effects) {
        // HP changes
        if (effects.hp) {
            const currentHp = game.state.get('hp') || 50;
        ...
        // Max HP changes
        if (effects.maxHp) {
            const currentMax = game.state.get('maxHp') || 80;
            const newMax = Math.max(1, currentMax + effects.maxHp);
            game.state.set('maxHp', newMax);
            
            // Also adjust current HP if maxHp decreased
            const currentHp = game.state.get('hp') || 50;
            if (currentHp > newMax) {
                game.state.set('hp', newMax);
            }
        ...
        if (effects.artifact) {
            ...
            artifacts.push({ id: effects.artifact });
            game.state.set('artifacts', artifacts);
            game.eventBus.emit('artifact:gained', { id: effects.artifact });
        }
