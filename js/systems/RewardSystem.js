...
class RewardSystem {
    ...
    applyImmediateArtifactEffects(artifact) {
        ...
        if (artifact.effects.maxHp) {
            const currentMax = this.state.get('hero.maxHp') || 80;
            const newMax = currentMax + artifact.effects.maxHp;
            this.state.set('hero.maxHp', newMax);
            
            // Also heal for the gained amount
            const currentHp = this.state.get('hero.hp') || 50;
            this.state.set('hero.hp', Math.min(newMax, currentHp + artifact.effects.maxHp));
