...
class RewardSystem {
    ...
    /**
     * Render a single reward card with rarity styling
     */
    renderRewardCard(card, isCorrupted) {
        ...
        return `
            <div class="reward-card type-${card.type} rarity-${rarityClass} ${shimmerClass}" 
                 data-card-id="${card.id}"
                 data-corrupted="${isCorrupted}">
    ...
    animateCardReveal(container) {
        const cards = container.querySelectorAll('.reward-card');
        cards.forEach((card, index) => {
            ...
            setTimeout(() => {
                ...
                // Extra flash for rare+
                if (card.classList.contains('rarity-rare') || 
                    card.classList.contains('rarity-legendary') ||
                    card.classList.contains('rarity-corrupted')) {
                    setTimeout(() => {
                        card.classList.add('rarity-flash');
                        // Emit sound cue
