...
class CorruptionSystem {
    ...
    setupEventListeners() {
        this.eventBus.on('corruption:gained', (data) => {
            try {
                // Support both raw number and object formats
                const amount = typeof data === 'number' ? data : (data?.amount ?? data?.value ?? Number(data) || 0);
    ...
    addCorruption(amount) {
        ...
        // Only set state if it hasn't been updated yet (compare with expected value)
        // This prevents double-writing when CombatScreen already set state before emitting
        const newValue = Math.min(100, Math.max(0, current + amount));
        
        // If current already includes the addition (CombatScreen pre-set), use current
        // Otherwise apply the addition
        const finalValue = current; // Trust the current state — caller likely already set it
        
        this.checkThresholds(Math.max(0, current - amount), current);
        this.eventBus.emit('corruption:changed', finalValue);
        this.updateUI();
    }
