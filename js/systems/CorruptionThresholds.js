...
export default class CorruptionThresholds {
    ...
    resolveChoice(event, choiceIndex, overlay, threshold) {
        ...
        if (effects.hp) {
            const current = this.state.get('hp') || this.state.get('hero.hp') || 50;
            const newHp = Math.max(1, current + effects.hp);
            this.state.set('hp', newHp);
            this.state.set('hero.hp', newHp);
        }
        
        if (effects.maxHp) {
            const currentMax = this.state.get('maxHp') || this.state.get('hero.maxHp') || 50;
            ...
            if (effects.maxHp > 0) {
                const hp = this.state.get('hp') || 50;
                this.state.set('hp', Math.min(newMax, hp + effects.maxHp));
